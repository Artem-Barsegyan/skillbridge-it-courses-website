[🌐 **Рабочая версия**](https://skillbridgesite.vercel.app/)

# SkillBridge - Образовательная платформа

Современная платформа для онлайн-курсов по дизайну и разработке на React.

## 🚀 Возможности
- **Главная страница** с видео-презентацией
- **Каталог курсов** с детальным описанием
- **Гибкие тарифы** (Standard/Pro)
- **Адаптивный дизайн** для всех устройств
- **Формы авторизации** с валидацией
- **FAQ раздел** с раскрывающимися вопросами
- **Отзывы студентов** в слайдере

## 🛠️ Технологии
- React 18 + React Router DOM
- SCSS/SASS с модульной архитектурой
- Video.js для видео-плеера
- Swiper.js для слайдеров
- Адаптивная верстка (mobile-first)

## 🎨 Дизайн
- **Цветовая схема**: Оранжевый (#FF9500) + оттенки серого
- **Шрифт**: Be Vietnam Pro
- **Компоненты**: Скругленные углы, современные карточки
- **Адаптивность**: От 320px до 1920px+

## 📱 Основные страницы
1. **Главная** (`/`) - Презентация платформы с видео, преимуществами, курсами и отзывами
2. **Курсы** (`/courses`) - Полный каталог курсов по дизайну и разработке
3. **Детали курса** (`/courses/:courseName`) - Подробная программа и материалы конкретного курса
4. **Тарифы** (`/pricing`) - Сравнение Standard и Pro планов с переключением периодов оплаты
5. **О нас** (`/about`) - История, достижения и цели платформы
6. **Контакты** (`/contacts`) - Форма обратной связи и контактная информация
7. **Авторизация** (`/login`, `/signup`) - Вход и регистрация

## 🚀 Быстрый старт
```bash
# Клонировать репозиторий
git clone https://github.com/ваш-username/skillbridge.git

# Установить зависимости
npm install

# Запустить проект
npm start
